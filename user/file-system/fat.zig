...
pub const Directory = struct {
    ...
    pub fn iterator(self: Directory) Iterator {
    ...
    pub const Iterator = struct {
        ...
        pub fn next(self: *Iterator, file_name: *[]u8) ?*Entry.Normal {
            var lfn_buffer: [256]u16 = undefined;
            var lfn_length: usize = 0;

            while (true) {
                defer self.fetch();
                const directory_entry: *Entry = @alignCast(@ptrCast(&self.cache_entry.data[self.byte_index]));

                if (directory_entry.normal.name[0] == 0)
                    return null;

                if (@as(u8, @bitCast(directory_entry.normal.attributes)) == @as(u8, @bitCast(Entry.Attributes.long_file_name))) {
                    const lfn_entry = &directory_entry.long_file_name;
                    var offset: usize = (lfn_entry.sequence.number - 1) * Entry.LongFileName.file_name_total_capacity;
                    ...
                    const file_name_1_len = for (0.., &lfn_entry.name_1) |i, c| {
                        if (c == 0x0000)
                            break i;
                    } else Entry.LongFileName.name_1_capacity;
                    ...
                    if (file_name_1_len < Entry.LongFileName.name_1_capacity)
                        continue;

                    const file_name_2_len = mem.indexOfScalar(u16, &lfn_entry.name_2, 0x0000) orelse Entry.LongFileName.name_2_capacity;
                    @memcpy(lfn_buffer[offset..].ptr, lfn_entry.name_2[0..file_name_2_len]);
                    offset += file_name_2_len;
                    if (file_name_2_len < Entry.LongFileName.name_2_capacity)
                        continue;

                    const file_name_3_len = mem.indexOfScalar(u16, &lfn_entry.name_3, 0x0000) orelse Entry.LongFileName.name_3_capacity;
                ...
                if (lfn_length > 0) {
                    ...
                    const name_length = std.mem.indexOfScalar(u8, &normal.name, ' ') orelse Entry.Normal.name_capacity;
                    @memcpy(file_name.ptr, normal.name[0..name_length]);

                    const extension_length = mem.indexOfScalar(u8, &normal.extension, ' ') orelse Entry.Normal.extension_capacity;
...
pub fn init(vbr_sector: usize, vbr: *const VolumeBootRecord) usize {
    ...
    const root_directory_sectors = math.divCeil(usize, vbr.root_directory_entries * @sizeOf(Directory.Entry), 512) catch unreachable;
