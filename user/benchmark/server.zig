...
pub fn main(args: []usize) !void {
    ...
    const spin_estimate_refinement_rate = 8;
    ...
    while (true) {
        ...
        while (spin_count < spin_limit) {
            var atleast_one_request: bool = false;
            for (rpc_buffers[1..], signals[1..], events[1..]) |rpc_buffer, *signal, *event| {
                atleast_one_request = handleDataClient(rpc_buffer, signal, event) or atleast_one_request;
            }
            if (atleast_one_request) {
                spin_estimate += @divTrunc(spin_count - spin_estimate, spin_estimate_refinement_rate);
                spin_count = 0;
            } else {
                spin_count += 1;
            }
        }
        spin_estimate += @divTrunc(spin_limit - spin_estimate, spin_estimate_refinement_rate);
